...
export default function DrrWtPortalPage() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      setData({
        ...
        tlttVins: mainJson.tlttVins || 0,
        repVins: mainJson.repVins || 0,
        tlttPercent: mainJson.tlttPercent || 0,
      });

      const defectsRes = await fetch(`${API_BASE}/api/drr-wt-portal-defects?${params.toString()}`);
  ...
  const loadVinList = async (status) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/api/drr-wt-portal-vins?${params.toString()}`);
  ...
  const pieData = [
    ...
    { name: 'Ушли в REP', value: Math.max(0, 100 - data.tlttPercent) },
  ];
          ...
          <div style={chartColumnStyle}>
            ...
            <div style={{ display: 'flex', gap: '15px', marginTop: '20px', flexWrap: 'wrap' }}>
              <div
                ...
                onClick={() => loadVinList('TS16')}
              >
                <div style={{ fontSize: '1.2rem', fontWeight: 600, opacity: 0.9 }}>Прошли TS16</div>
              ...
              >
                ...
                <div style={{ width: '70%', height: '2px', backgroundColor: 'rgba(255,255,255,0.3)', margin: '10px auto' }} />
                <div style={{ fontSize: '4rem', fontWeight: 900, lineHeight: 1 }}>{data.tlttVins}</div>
              </div>
        ...
        >
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              <h3 style={{ margin: 0, fontSize: 20, fontWeight: 700 }}>VIN ({vinListStatus})</h3>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  <thead>
                    <tr>
                      ...
                      <th style={{ textAlign: 'left', padding: '8px', borderBottom: '1px solid #E5E7EB' }}>Время события</th>
                    </tr>
                  </thead>
                  <tbody>
                    {vinList.map((item, idx) => (
                      <tr key={idx}>
                        ...
                        <td style={{ padding: '8px', borderBottom: '1px solid #F0F0F5' }}>
                          {item.event_time ? new Date(item.event_time).toLocaleString('ru-RU') : '—'}
                        </td>
                      </tr>
                    ))}
                  </tbody>
