...
export default function TLMapPage() {
  ...
  // Обработка данных: группировка по зонам и spec, детали по зоне+spec
  const processedData = useMemo(() => {
    const zonesMap = {};
    const detailsMap = {}; // Ключ: `${zone}_${spec}`
    ...
    rawData.forEach(row => {
      ...
      // Ключ для деталей: зона + spec
      const detailsKey = `${zone}_${spec}`;
      if (!detailsMap[detailsKey]) {
      ...
      // Группировка по зонам для кнопок
      if (!zonesMap[zone]) {
        zonesMap[zone] = {};
      }
      if (!zonesMap[zone][spec]) {
        zonesMap[zone][spec] = 0;
      }
      zonesMap[zone][spec]++;
    });
    ...
    zones.sort((a, b) => {
      const indexA = ZONE_ORDER.indexOf(a.zoneName);
      const indexB = ZONE_ORDER.indexOf(b.zoneName);
  ...
  const filteredZones = processedData.zones.map(zone => ({
    ...
    items: zone.items.filter(item => {
      const itemModel = item.label.split('.')[0];
      return selectedModels.includes(itemModel);
    })
  }));
  ...
  // При клике на кнопку передаем zoneName и label
  ...
  if (error) {
    return (
      <div style={containerStyle}>
        <div style={{ textAlign: 'center', color: '#DC2626', fontSize: 16, padding: 40 }}>
          {error}
        </div>
      </div>
    );
      ...
      <div style={kanbanContainerStyle}>
        {zonesWithTotal.map((zone) => (
          <div key={zone.zoneName} style={zoneColumnStyle}>
            <div style={zoneTitleStyle}>{zone.zoneName}</div>
